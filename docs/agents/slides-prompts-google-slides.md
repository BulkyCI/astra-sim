# Google Slides build prompts

Ten slide prompts for a slides-generating agent. Paste **Block 0** once to
set the deck up, then paste slide blocks one at a time in order. Every
block is self-contained: the assertion, the visual, the data and the
layout are all in it, so nothing has to be inferred.

Prepared by Joe Fang, in collaboration with Zechen Ma. Audience: Yashar
Ganjali, 9 September 2026.

The deck follows the results-first framing: run #117 is the centre and
FORGIVE is future work. Source documents are
`slides-2026-09-results-first.md` for the argument and `figure-data.md`
for every number.

---

## Block 0. Deck setup, paste first

~~~
Create a new 16:9 presentation, 1920 x 1080 px, titled
"Phase-aware bounded loss at LLM scale".

DESIGN SYSTEM. Use these tokens for every slide. Do not introduce any
other colour, typeface, or decorative element.

Colours, two only:
  ink        #141413   (all text, all rules, all chart marks, dark panels)
  paper      #faf9f5   (all backgrounds, and text on top of ink panels)
  Use pure white #ffffff only for text sitting on an ink panel.
Because the palette has no accent colour, encode every chart distinction
with an ink opacity ramp, never with hue:
...
use "Anthropic Mono", falling back to Arial. The brand body size is 12px
for a web page; this deck is projected, so scale the whole ramp by 1.6x
and keep the brand's proportions and tracking:
...
Spacing: use only 2, 4, 8, 12, 16, 22, 58, 68 px. Slide margin 68px on
all sides. Gap between a headline and its content 22px. Gap between
columns 58px.

Corners: 8px on small chips, 16px on cards, 24px on full panels.
Shadows: almost invisible. rgba(0,0,0,0.01) 0 2px 2px,
rgba(0,0,0,0.02) 0 4px 4px, rgba(0,0,0,0.04) 0 16px 24px. Never a
heavier shadow, never a glow, never a gradient.

DECK RULES, apply to every slide without exception:
1. The headline is an assertion: a full sentence with a verb that states
   the finding. Never a topic label like "Results" or "Methodology".
2. Exactly one visual per slide. It occupies at least half the slide.
3. At most three supporting facts, each one line. Never a bullet list of
   more than three items. Never a paragraph.
4. A single closing line at the bottom of the slide, set in ink at 55%,
   answering "so what". Label it nothing; just place it.
5. Numbers are the hero. Set any headline figure at 58px weight 700 and
   let it sit in white space. Never bury a number inside a sentence when
   it can stand alone.
6. No clip art, no icons, no stock photography, no emoji, no 3D, no drop
   shadows on text, no rounded "pill" callouts other than the chip style
   defined above.
7. Charts carry no chart junk: no legend boxes, no axis borders on all
   ...
   beside its line or bar.
8. Slide numbers bottom right, 14px, ink at 30%.

Confirm the setup, then wait for slide prompts.
~~~
...
~~~
SLIDE 1 of 10. Title slide. Full-bleed ink panel (#141413), all text in
paper (#faf9f5).

Title, 58px bold, upper left, starting at the 68px margin, wrapped to two
lines:
  "Phase-aware bounded loss at LLM scale"

Directly beneath it, 34px, weight 400, paper at 70% opacity:
  "What it buys, and where it stops"

Bottom left, 20px, three lines, tight:
  "Joe Fang, in collaboration with Zechen Ma"
  "Progress review for Yashar Ganjali"
  "9 September 2026"

Bottom right, a single horizontal strip of three statistics, each a
figure above a label, 58px bold figure and 16px label, separated by 58px
of space:
  "3.91 %"   / "shorter training window, 16 matched seeds"
  "153 ms"   / "off the worst all-reduce of the episode"
  "0.93"     / "correlation with trims prevented"

No other content. No logo. No decorative rule.
...
## Block 2. The logic of the whole programme

~~~
SLIDE 2 of 10. This is the argument slide. It must show a chain of
reasoning, not a list.

Headline, 34px bold:
  "Each result decided what we ran next"

Visual: four cards in a single horizontal row, equal width, 16px corner
radius, 1px ink border, paper fill, connected left to right by thin ink
arrows (2px, ink at 30%) placed in the 58px gaps between them. Each card
carries a monospace step number at 28px in ink at 30% at its top left,
then a bold 20px line, then two lines of 20px regular text.

Card 1, number "01", bold line "The May question":
  "Phase-aware bounded loss worked on four nodes with loss we injected by"
  "hand. Does it hold where the network itself makes the loss?"

Card 2, number "02", bold line "Build the fabric":
  "No public backend modelled it, so we built one: packet trimming, PFC"
  "off, 64 ranks, loss produced by a real incast."

Card 3, number "03", bold line "It works, and we know why":
  "3.91 % of training time across sixteen seeds. The saving tracks the"
  "packet trims prevented, not the gradient bytes discarded."

Card 4, number "04", bold line "And we know where it stops":
  "The value comes from the recovery scheme. Change go-back-N to"
  "selective repeat and 3.9 to 11.1 % becomes 0.78 %."

Closing line, ink at 55%, 20px, centred under the row:
...
~~~
SLIDE 3 of 10. Timeline. It must read chronologically at a glance.

Headline, 34px bold:
  "Six of the seven weeks went into the instrument"

Visual: a horizontal timeline spanning the full content width. The axis is
a 2px ink rule with ticks at 20 July, 31 July, 11 August, 22 August,
1 September and 9 September, labelled at 16px beneath.

ABOVE the axis, four stacked horizontal bands, each an ink-12 rectangle
with a 1px ink-30 border, 24px tall, labelled inside at 16px bold. Bands
span these date ranges:
...
Between the bands and the axis, place small ink dots on the axis with
16px labels above them, joined by 1px ink-30 leader lines:
...
BELOW the axis, four solid ink diamonds on 2px ink stems, with bold 20px
labels and 16px sublabels, for the cluster runs:
   1 Sep  "run #117"  /  "16 seeds, go-back-N"
   6 Sep  "run #120"  /  "8-cell regime map"
   7 Sep  "run #121"  /  "FORGIVE"
   8 Sep  "run #122"  /  "budget sweep"

Closing line, ink at 55%:
...
## Block 4. The headline result

~~~
SLIDE 4 of 10. The main result. The chart is the slide.

Headline, 34px bold:
  "Thirteen of sixteen seeds improve, and the spread is the finding"

Visual: horizontal dumbbell chart occupying the left two thirds. One row
per seed, sorted by relief descending. Horizontal axis is the 20-step
training window in milliseconds, 6500 to 7850, ticks every 200,
gridlines in ink-12. Each row: an ink-30 filled circle at the baseline
value and an ink-100 filled circle at the policy value, joined by a 3px
line. On rows where the policy is worse, draw the joining line in ink-55.
Seed labels at 16px on the left, relief percentage at 16px on the right.
...
Right third: an ink panel (#141413), 24px radius, paper-coloured text:
...
## Block 5. Why it works

~~~
SLIDE 5 of 10. The mechanism. Two scatter panels side by side, equal
width, sharing one vertical axis label.

Headline, 34px bold:
  "The saving tracks trims prevented, not gradient bytes discarded"

Shared vertical axis for both panels, labelled "milliseconds saved on the
window", range -600 to 900, gridlines ink-12, a heavier ink-30 line at
zero.
...
## Block 6. What the phase bound costs

~~~
SLIDE 6 of 10. The price of the safety property. A comparison of three
quantities, no chart junk.
...
To the right of each bar, the relief at 34px bold with its interval at
16px beneath:
...
Speaker note: "Both shedding arms drop messages at the sender. They differ
only in whether the critical steps are protected."
~~~

---

## Block 7. The boundary

~~~
SLIDE 7 of 10. Own the scope of the result. This slide must not read as an
apology.

Headline, 34px bold:
  "The gain is a property of the recovery scheme, and we measured its edge"
...
range from its low to its high value, showing the go-back-N cost as a
multiple of the selective-repeat cost. Gridlines at 1x, 10x, 100x and
...
14px on a second line:
  ...
  "trim ratio"                       110x to 520x   "2.2 to 10.4 against 0.02"
  "retransmitted per offered byte"    54x to 312x   "7x to 25x against 0.08x to 0.13x"
  "burst drain time"                  15x to  78x   "423 to 1798 ms against 23 to 29 ms"
  "all-reduce span, burst step"       12x to  53x   "205 to 935 ms against 17.6 ms"
  "20-step window"                   3.7x to 5.6x   "4503 to 6795 ms against 1210 ms"
...
Right third: an ink panel with paper text:
  ...
  "Rule asked for a burst excess of 20 % of the window. Worst cell reached
  0.62 %."
...
## Block 8. The four May questions

~~~
SLIDE 8 of 10. Close the loop on what we set out to settle.
...
Card 02, chip "CLOSED", question "Compute and transport interleaving":
  "Chakra traces overlap 5.4 ms of compute per node with the window, so we
  report exposed communication time. This is the confound that made us
  stop quoting the May headline."

Card 03, chip "SCOPED", question "CLR identification":
  "A simulator with no gradients cannot host a detector, so the schedule is
  pinned from independent literature. What we can now do is price it: the
  mask costs 3.2 points of time and holds to the byte in the ledger."
...
Card 04, chip "CLOSED", question "Centralized against ring":
  ...
  ratio, multiplying it 2.7x and 5.5x. Hub-and-spoke pressure at one NIC is
  our fan-in 7 cell, the worst on the map."
...
~~~
SLIDE 9 of 10. FORGIVE as ongoing work, with the preliminary numbers.
Mark it preliminary in the visual itself, not only in the notes.
...
band from 0.7 % to 3.3 % in ink-12 and label it at 14px "tolerance range
MLT reports". Draw a dashed ink-30 vertical line at 10 %, labelled "MLT at
a quality target".
...
third of every gradient, and no published tolerance result reaches there."
~~~
...
~~~
SLIDE 10 of 10. Three requests. Full-bleed ink panel (#141413) with all
text in paper (#faf9f5), matching slide 1 so the deck closes where it
opened.

Headline, 34px bold, paper:
  "Three things I would like from you"

Visual: three equal columns separated by 1px paper-30 vertical rules. Each
column has a monospace number at 28px in paper at 40%, a bold 20px title,
and three to four lines of 20px body.

Column 01, "A GPU collaborator":
  "Export the forgiven byte ranges, zero those elements in a PyTorch DDP
  communication hook, train a 1B-class model against an unmodified run at
  budgets 0.1 to 0.4. Eight GPUs for one to two weeks. It is the only
...
Column 02, "A view on the controller":
  ...
  depend on it, but the paper's currency might. Is it worth three to four
  weeks to implement NSCC, the controller the Ultra Ethernet specification
  defines?"
...
No closing line on this slide. No slide number.
...
## Optional Block 11. Threats to validity, backup slide

Build this only if a backup section is wanted. It is the slide that
protects the deck under questioning.

~~~
BACKUP SLIDE. Same design system. Headline, 34px bold:
...
regular. Rows:
  ...
  "per-rank p99"
  "Not a result. Averages -4.9 % across the same sixteen seeds, CI [-19.6,
  +9.8]. It is the top three of 320 samples and one path collision moves
  it by half."
  ...
  "Ran unmatched: the profile name entered the selection hash. Fixed in
  commit 63ef7c2, not yet re-run."
...
- Slides 4, 5, 7 and 9 carry the argument. If the generator produces a
  weak visual on any of them, rebuild that slide rather than adding text
  to compensate.
- If a chart cannot be drawn natively, every data point is in
  `figure-data.md` and reference SVGs are in `figures/`. Rebuild rather
  than screenshot: those SVGs use a different palette and will clash with
  this design system.
- The deck deliberately has no methodology slide. Setup facts live in the
  speaker notes and in the backup slide, which is where they belong for
  this audience.
- If ten slides is too many, cut slide 8 first and fold its two closed
  rows into slide 3's closing line. Never cut slide 7.
