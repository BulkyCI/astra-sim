...
The direct-DP profiles run a **4:3-oversubscribed leaf-spine fabric** (8 hosts, 6 spines). This ratio is the congestion knee, chosen from two measured endpoints: at 2:1, synchronized DP puts the fabric into sustained 2× overload where go-back-N recovery under trimming collapses goodput and the baseline arm cannot finish; at 1:1, symmetric all-to-all is capacity-matched (each sender's NIC divides across its concurrent flows, so receivers see roughly line rate), the network stops being the bottleneck, and a bounded-loss transport has nothing to relieve. At 4:3 the DP phase stays network-bound — a 33% structural excess plus ECMP-collision hotspots produce a heavy latency tail — while DCQCN can still hold the system at the knee, which is exactly the regime where shedding a bounded fraction of gradient traffic buys super-linear relief. The 16-rank flagship keeps its 2:1 fabric as the hard oversubscribed condition (its dp=2 phase is small enough to grind through); a full oversubscription sweep returns once the transport recovers trimmed packets selectively instead of rewinding the whole window.
