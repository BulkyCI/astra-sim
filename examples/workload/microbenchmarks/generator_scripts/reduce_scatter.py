...
def generate_reduce_scatter(npus_count: int, coll_size: int, path="./") -> None:
    """
    Generate Reduce-Scatter ET files.
    ...
    for npu in range(npus_count):
        et_filename = os.path.join(et_path, f"reduce_scatter.{npu}.et")
...
def main():
    ...
    # generate ET files
    generate_reduce_scatter(npus_count=npus_count, coll_size=coll_size, path="./")
