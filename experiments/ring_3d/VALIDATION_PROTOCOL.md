...
| Direct-DP incast proof point | `profiles/llama3_70b_32_direct.json`, $DP=4$ with the per-group direct all-reduce override | Establish that DP fan-in, not only the injected microburst, drives congestion | One fixed-seed pair with the best-effort congestion gate. If no queue/trim response appears at any achievable fan-in, the incast thesis fails on this substrate — stop before building the fan-in matrix. |
| DP fan-in sweep | `profiles/llama3_70b_64_direct{1,2,4,}.json`, $DP=8$ with windows $\{1,2,4,7\}$ | Locate the congestion knee against receiver fan-in with transport concurrency as the only variable | One fixed-seed pair per point under the best-effort congestion gate; `fan_in_sweep.py` renders pressure and relief against fan-in. The claim under test is that policy relief widens past the knee, not that any single point improves. |
