...
The reproducible TP/PP/DP Ring experiment lives in [experiments/ring_3d](experiments/ring_3d). It generates Chakra ET workloads with explicit `parallelism_domain` attributes, native Ring All-Reduce process groups, pipeline send/receive pairs where $PP>1$, and typed DP gradient buckets. Its typed physical-network model supports both a two-stage Clos and a host-attached switch ring. The smoke profile is $TP=2$, $PP=2$, $DP=2$; the CI-scale Llama 3 70B-class profile is $TP=8$, $PP=1$, $DP=2$ over 16 ranks.
...
The CI also runs `llama3_70b_16.json`: a 70B-class FP16 gradient-bucket workload over $TP=8$, $PP=1$, and $DP=2$. It uses one representative 1 GiB typed DP All-Reduce bucket per rank and step, 4 KiB RoCE payloads, and seven simultaneous 128 MiB cross-rack RDMA microbursts toward rank 8. Five matched baseline/policy pairs run independently, each with a 330-minute guard and a 150-minute cap for each ns-3 process within GitHub Actions' six-hour job limit; their verified artifacts are then combined into one five-seed Markdown report. Every pair fails closed unless every run records background traffic and the signature of the regime it modeled: a nonzero queue peak and a trim or rejection count on this best-effort fabric (PFC is off), or a completed PFC pause interval on a lossless profile. Two additional concurrent CI jobs run the same structural 100B, 256-card $TP=8$, $PP=4$, $DP=8$ workload on Clos and host-attached switch-ring fabrics. Every completed or partial structural run produces an auditable Markdown report and reproducibility bundle. These are topology/workload characterizations, not claims to replay a complete framework training stack or establish a policy effect without matched baselines.
