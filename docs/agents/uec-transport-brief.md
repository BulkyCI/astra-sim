<!-- Produced 2026-09-05 by a Sonnet research agent from the UEC v1.0.1/v1.0.2 specification PDFs and the cited papers; repo claims verified against file:line by the session lead. CORRECTION: the agent wrongly concluded that v1.0.3 did not exist; it does (released 2026-07-16). Every claim below was re-verified against v1.0.3 in uec-1.0.3-reconciliation.md: all hold with shifted page numbers except UET_TRIMMED_ACK, which v1.0.3 removes (a trimmed ACK no longer generates a NACK). Use the reconciliation note for v1.0.3 page numbers and the recommended NSCC/RCCC defaults. -->
...
own specification PDFs (fetched and read directly, not from memory) or from other cited
web sources retrieved 2026-09-05. Section/page numbers refer to the PDF page unless noted.
Repository claims are sourced to file:line in `/config/repositories/astra-sim`.

## Part 1 — UEC transport facts

### 1. Spec status, structure, and the "1.0.3" citation

UEC published Specification v1.0 on 2025-06-11, v1.0.1 on 2025-09-05, and v1.0.2 on
2026-01-28, all publicly downloadable [S1][S2][S3]. As of today (2026-09-05) **no v1.0.3
exists** — the release-notes page inside v1.0.2 lists only 1.0 → 1.0.1 → 1.0.2 [S3 p.564],
and a targeted search found no 1.0.3 artifact. **Our repo's citation of "UEC 1.0.3" is
therefore an unverifiable version number**, not something I could retrieve. Treat this as
a citation defect, not necessarily a content error:

- **Section 4.1 "Packet Trimming"** (Network Layer chapter) is real and identical in
  location across v1.0.1 and v1.0.2 (page 451, subsections 4.1.1–4.1.7) [S2 p.170][S3 p.451].
- **Section 3.6.4.4 "Packet Trimming"** (inside the Congestion Management Sublayer chapter,
  a *second*, CC-oriented treatment of the same feature) is also real, at page 347–349
  of v1.0.2 [S3 p.347–349]. Our repo's two different citations ("§4.1" and "§3.6.4.4") are
  each individually correct section numbers in the real spec — they are two different
  sections that both discuss trimming from different angles (network-layer switch
  behavior vs. congestion-management framing). The version number attached to them
  ("1.0.3") is what is wrong; it should read v1.0.1 or v1.0.2 depending on when the text
  was drafted.

UEC's four transport sublayers, confirmed in the TOC and body text [S3 pp.3–4, p.123–130]:
...
Four delivery modes, all confirmed at [S3 p.231–233]:
...
- **ROD** (Reliable Ordered Delivery): delivered exactly once, in the order SES originally
  ...
  all packets starting from the first missing PSN."** [S3 p.232]. So go-back-N is explicitly
  part of UET — but only for the ordered mode, which by design forgoes multipath spraying.
- **RUDI** (Reliable Unordered Delivery for Idempotent operations): delivered *at least
  ...
  detection and per-packet RUDI responses [S3 p.232–233]. RUDI is still **reliable**, not
  a "deliver what arrived" mode — this corrects a framing in our research question (see §7).
- **UUD** (Unreliable Unordered Delivery): a basic best-effort datagram service, no ACKs,
  not congestion-controlled by UET-CC [S3 p.233].
...
[S3 p.281–283, §3.5.11.14/3.5.12.3.1/3.5.12.4.3]. Retransmission is triggered by three
independent mechanisms, all "MUST": trimming (single-RTT), explicit NACK, and RTO as
backstop [S3 p.294, Table 3-60]. NACK codes `UET_TRIMMED` (0x01), `UET_TRIMMED_LASTHOP`
(0x02), and `UET_TRIMMED_ACK` (0x03) are defined exactly as our code assumes [S3 p.288–289,
Table 3-58]. RTO uses exponential backoff: `RTO_TIMER = RTO_INIT_TIME * 2^retry_count`,
range 0–8 s [S3 p.287–288, 295]. Packet spraying places each packet on a pseudo-randomly
chosen entropy value from a pool of 64–256 entropies, reacting to ECN/trim feedback per
path [S3 p.357, §3.6.5.2]. A PDC (Packet Delivery Context) is a dynamically established,
ephemeral FEP-to-FEP connection created with zero round-trip startup cost and torn down
when idle [S3 p.234, 236].

### 3. Packet trimming — confirmed and corrected

Verified against the spec text directly [S3 pp.451–459, §4.1 and pp.347–349, §3.6.4.4]:
...
- Trimmed size floor is `MIN_TRIM_SIZE`; Table 4-1 gives **24 B for UET/UDP/IP** (UDP 8 B +
  PDS request header 16 B) [S3 p.456] — this matches our repo's default `min_trim_size_bytes
  = 24` exactly.
- TC_med bandwidth share: **"Fair-queueing allows trimmed packets to consume no more than
  ...
  large incasts."** [S3 p.455]. Our repo's "25% WDRR recommended, 50% cap" is verified correct.
- **Back-to-sender is explicitly out of scope**: "This specification focuses only on
  ...
  source... is not part of this specification." [S3 p.451]. Confirms our repo's "ftd is
...
- Trimming is a congestion **signal**, but a conditional one for the last hop: **"[a
  ...
  also enabled*, because RCCC can handle last-hop congestion by itself."** [S3 p.349]. The
  exclusion in the spec is conditioned on RCCC being present — see the gap this creates in
  Part 2 §4.

### 4. Congestion control: NSCC, RCCC, and comparison to DCQCN / our mode 1
...
`Rcvd_Bytes`, which "clocks" `inflight` down [S3 p.377, §3.6.13]. It is a dual-signal
...
which sets `cwnd` from measured achieved goodput rather than halving it [S3 pp.377–389,
...
UET_TRIMMED_LASTHOP and rccc == FALSE): adjust_cwnd = TRUE"` [S3 p.385–386]. NSCC's spec
text is explicit about *why* it avoids a rate-based design: **"in contrast, in a
rate-based approach such as DCQCN, lack of feedback is implied to be a sign of appropriate
network operation, and the rate is increased. This approach works in lossless networks,
where PFC kicks in as a backstop, but in best-effort networks where packets may be
silently discarded due to congestion, increasing throughput in response to a lack of
congestion feedback results in poor performance."** [S3 p.358, §3.6.5.3]. NSCC descends
directly from the SMaRTT-REPS and STrack research algorithms, both cited by name in the
spec's own references [S3 p.68, refs 9–10] [S4][S5].
...
[S3 pp.391–399, §3.6.14]. This is the explicit UET mechanism for incast: **"To deal with
incast, RCCC leverages information available at the destination to make optimal,
instantaneous changes to the transmission rate of each of the active sources."** [S3 p.391].
When RCCC is enabled, ECN on the last hop is disabled by recommendation, because RCCC
already manages that hop directly and NSCC would otherwise double-react [S3 p.392].
...
- **SMaRTT-REPS** [S4] (arXiv:2404.01630, Bonato et al., April 2024) — the paper the spec
  cites as NSCC's direct source [S3 p.68]. Built on **htsim** (not ns-3), evaluated on
...
- **REPS** [S8] and **STrack** [S5] are the other two papers the spec cites by name for
  adaptive spraying and multipath reliability [S3 p.68]; not deep-read beyond
  abstract/citation record — treat any numeric claim from them as **unverified**.
- I found **no dedicated ns-3 UET module** in public circulation as of 2026-09-05.
...
apples-to-apples external number for "the cost of a lost packet on a UEC fabric" that I
can cite** — only qualitative confirmation that trimming plus NSCC/RCCC keeps that cost
small relative to a no-CC or DCQCN-only baseline.
...
This is where our research framing needs correcting. **RUDI is not the unreliable mode** —
it is *reliable* (at-least-once) delivery for idempotent operations; every RUDI request
still gets NACK'd and retried on loss [S3 p.232–233]. The mode that actually matches
"deliver what arrived, no repair" is **UUD** (Unreliable Unordered Delivery): "a basic
datagram service. An unreliable datagram service enables best-effort delivery... There is
no acknowledgement for the unreliable delivery mode." [S3 p.231, 233]. UUD is explicitly
**not controlled by UET-CC**, and the spec places the burden of avoiding self-inflicted
congestion on the application if UUD shares a traffic class with RUD/ROD [S3 p.233]. UUD
is the natural spec-level home for a bounded-loss gradient transport's data plane — it is
a genuine "fire it and accept what arrives" primitive, unlike RUDI, which still repairs
everything, just without ordering or duplicate suppression guarantees.
...
1. **Drop the `!lastHop` guard at `rdma-hw.cc:635`.** The spec only excludes last-hop
   trims from the CC signal when RCCC is present [S3 p.349]. We have no RCCC, so leaving
...
2. **Rescale `RATE_AI`/`RATE_HAI`/`MIN_RATE` for 400 Gb/s.** `generate.py:1207-1208`
   ...
   would be very slow. I found no UEC- or vendor-published multiplier for this — "scale
   roughly like KMIN_MAP/KMAX_MAP" is my own heuristic, **unverified**, and should be
...
[S3 p.358]. DCQCN-with-trims is a fine baseline for "does some CC beat no CC," but "this
...
consistent with our own SR-vs-go-back-N canary (SR ~4x faster, loss "nearly free"), but I
cannot responsibly convert either paper's numbers into a claim about our specific `W` or
...
- [S3] Ultra Ethernet Consortium, *Ultra Ethernet Specification v1.0.2* (PDF, 2026-01-28),
  https://ultraethernet.org/wp-content/uploads/sites/20/2026/01/UE-Specification-1.0.2-1.pdf — accessed 2026-09-05. (Page numbers above refer to this PDF's printed page numbers, which match its PDF page numbers.)
...
- [S7] General DCQCN literature summary via web search (slow convergence / PFC-dependence)
  ...
  text [S3 p.358], which is the authoritative citation used above.
...
- Whether any UEC member or third party has published a public **ns-3** UET module —
  I found none; this is a negative result, not a proof of nonexistence.
...
- A single authoritative primary source for "DCQCN's known weaknesses (slow convergence,
  PFC dependence)" as a standalone claim — I relied on the NSCC spec's own contrast text
  [S3 p.358] as the strongest primary source found; broader DCQCN critique literature
  exists but was not individually pinned to a citation here.
...
- The exact defensible multiplier to rescale `RATE_AI`/`RATE_HAI`/`MIN_RATE` for a
  400 Gb/s link in this codebase — I found no UEC or vendor-published value; the
  "scale like KMIN_MAP/KMAX_MAP" heuristic in Part 3(a) is my own inference, not a sourced
